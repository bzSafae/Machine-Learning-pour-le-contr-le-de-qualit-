...
Y = df['Pass/Fail']  # Remplacer 'target' par le nom de la colonne cible

# Normaliser les données (le PCA est sensible à l'échelle des données)
scaler = StandardScaler()
X_scaled = scaler.fit_transform(X)

# Appliquer le PCA
pca = PCA(n_components=2)  # Choisir le nombre de composantes principales à conserver
...
# Afficher la nouvelle forme des données après la réduction de la dimensionnalité
print("\nNouvelle forme des données après PCA :", X_pca.shape)

# Sauvegarder les données transformées dans un nouveau DataFrame
df_pca = pd.DataFrame(X_pca, columns=[f'PC{i+1}' for i in range(X_pca.shape[1])])
df_pca['target'] = y  # Ajouter la colonne cible au DataFrame transformé

# Sauvegarder le DataFrame avec les données PCA
df_pca.to_csv("pca_data.csv", index=False)
print("Données transformées sauvegardées dans 'pca_data.csv'")
